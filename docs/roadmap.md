...
> 历史记录：以下内容针对已删除的 Rust OCR 引擎，不再是当前计划。当前实现和验证见 [`../arale_onnx_v1/README.md`](../arale_onnx_v1/README.md)。
...
5. **性能**：比 Python 参考慢约 1.4×（同一批 30 页：Python 83.3 s、本引擎 119.9 s）。
   慢的**不是**束搜索策略（改成 5.x 语义只多 7%），而是 ONNX 解码器**没有 KV cache**：
   每一步都把整个前缀重算一遍（HF `generate` 有 cache），而且每步固定 batch=4。
   优化方向明确：导出带 `past_key_values` 的解码器。

   顺带纠正：本文档早前写过「新束搜索比旧早停版慢约 3×」——**那是错的**，
   是拿并发跑多份引擎时的日志推的。同机同批 30 页实测：旧早停 111.9 s、新 5.x 语义 119.9 s。
